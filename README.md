...
| **模型管理** | 独立一页：列出每个模型目录里**有什么文件、多大**（含你自己建的目录与高级选项里追加的目录），可搜索、可一键打开。**只列模型本身** —— 伴生的 `.civitai.info` / `.preview.png` / `.json` / `.sha256`、以及上游自带的占位说明会被自动收起来（收掉了多少会写在界面上） |
...
搜索也是这条主线的延伸：**数据层给全量，显示层才裁剪**。卡片一次只画前 50 项是为了界面不卡，
但**搜索是在全部文件里搜**的 —— 磁盘上有几百个 LoRA 时，敲名字照样能翻到排在第 60、第 100 位的那一个。
（如果反过来、先把数据砍到 50 项再搜，就会变成"模型明明躺着、搜索却命中 0 张卡"——`v0.35` 修掉的正是这个。）

同一页还有一条同样"只错一头"的取舍：**该显示什么，也得一分为二**。真实目录里 `xxx.safetensors`
旁边常常挨着 `xxx.civitai.info`、`xxx.preview.png`、`xxx.json`，老下载还会带 `xxx.sha256`
和上游的 `Put LoRA here.txt`（一个 814 项的 Lora 目录里有 593 项不是模型）。`v0.36` 起把它们收起来，
但方向只有一个 —— **错误只能是"多显示一个附属文件"，绝不能是"少显示一个模型"**：
只有"名字带明确标记"、或"元数据后缀且旁边真有同名模型"的才收，拿不准则保留
（`interrogators\model.json` 这种就是被正确留下来的）。数据层依旧是全量，卡片头的计数与体积
依然是磁盘事实，只是画出来的那一份换成了 `Visible()` 的产出。
